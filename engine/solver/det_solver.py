...
class DetSolver(BaseSolver):
    ...
    def fit(self, ):
        ...
        if self.last_epoch > 0:
            ...
            test_stats, coco_evaluator = evaluate(
                ...
                self.evaluator,
                self.device,
                epoch=self.last_epoch  # ✅ 修复1：断点续训评估时，传入上次训练的轮次
            )
            for k in test_stats:
                best_stat['epoch'] = self.last_epoch
                best_stat[k] = test_stats[k][0]
                top1 = test_stats[k][0]
                # ✅ 核心修复：删除这行打印，避免输出 best_stat: {...}
        ...
        for epoch in range(start_epoch, args.epoches):
            ...
            test_stats, coco_evaluator = evaluate(
                ...
                self.evaluator,
                self.device,
                epoch=epoch  # ✅ 修复2：训练循环中评估时，传入当前epoch轮次（核心修复）
            )
    ...
    def val(self, epoch=0):  # ✅ 修复3：val函数增加epoch默认参数，兼容所有调用场景
        self.eval()

        module = self.ema.module if self.ema else self.model
        test_stats, coco_evaluator = evaluate(module, self.criterion, self.postprocessor,
                self.val_dataloader, self.evaluator, self.device, epoch=epoch)  # ✅ 修复4：val函数评估传参
