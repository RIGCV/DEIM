...
class LightConvBNAct(nn.Module):  # 轻量卷积块：1x1卷积 + 深度可分离卷积
    def __init__(self, in_chs, out_chs, kernel_size, groups=1, use_lab=False):  # 初始化
        ...
        self.conv2 = ConvBNAct(out_chs, out_chs, kernel_size=kernel_size, groups=out_chs, use_act=True,
                               use_lab=use_lab)  # 深度卷积
...
class HG_Block(nn.Module):  # HGNet基本块：多分支叠加+聚合
    def __init__(self, in_chs, mid_chs, out_chs, layer_num, kernel_size=3, residual=False,
        ...
        if agg == 'se':  # SE聚合方式
            aggregation_squeeze_conv = ConvBNAct(total_chs, out_chs // 2, kernel_size=1, stride=1,
                                                 use_lab=use_lab)  # 降维
            aggregation_excitation_conv = ConvBNAct(out_chs // 2, out_chs, kernel_size=1, stride=1,
                                                    use_lab=use_lab)  # 升维
        ...
        if pretrained:  # 需要预训练
            ...
            try:  # 尝试加载
                ...
                if os.path.exists(model_path):  # 本地存在
                    ...
                    if rank0:  # rank0负责下载
                        print(
                            GREEN + "If the pretrained HGNetV2 can't be downloaded automatically. Please check your network connection." + RESET)  # 提示
                        print(
                            GREEN + "Please check your network connection. Or download the model manually from " + RESET + f"{download_url}" + GREEN + " to " + RESET + f"{local_model_dir}." + RESET)  # 指导
                        state = torch.hub.load_state_dict_from_url(download_url, map_location='cpu',
                                                                   model_dir=local_model_dir)  # 下载
